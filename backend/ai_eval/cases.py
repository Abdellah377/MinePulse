...
from app.ai.contracts import ConfidenceLevel, DiagnosisStatus, EvidenceKind, EvidenceRequestType, TriggerType
...
EVALUATION_CASES: dict[str, EvaluationCase] = {
    "clear_equipment_failure": EvaluationCase(
        ...
        expected_diagnosis_status=DiagnosisStatus.PROBABLE,
    ...
    "ambiguous_stop": EvaluationCase(
        ...
        forbidden_concepts=["confirmed mechanical failure", "hydraulic failure", "engine failure"],
        expected_reliable_root_cause=False,
        expected_diagnosis_status=DiagnosisStatus.INCONCLUSIVE,
    ...
    "causal_loader_bottleneck": EvaluationCase(
        ...
        expected_evidence_tools=["fleet_snapshot", "cycle_performance", "loading_context"],
        expected_evidence_kinds=[EvidenceKind.FACT, EvidenceKind.DERIVED_METRIC],
    ...
    "production_operational_bottleneck": EvaluationCase(
        case_id="production_operational_bottleneck",
        description="Production shortfall requiring an operational bottleneck explanation.",
        equipment_code="TRK-001",
        trigger_type=TriggerType.PRODUCTION_DEVIATION,
        ground_truth=EvaluationGroundTruth(
            label=GroundTruthLabel.PRODUCTION_BOTTLENECK,
            summary="A fleet, cycle, loading, waiting, or downtime mechanism must explain the shortfall.",
            reviewer_notes="Reject a conclusion that only repeats that production is below target.",
        ),
        expected_evidence_tools=[
            "shift_production",
            "fleet_snapshot",
        ...
        expected_evidence_kinds=[EvidenceKind.FACT, EvidenceKind.DERIVED_METRIC],
        expected_concept_groups=[["cycle", "loader", "loading", "waiting", "downtime", "availability", "bottleneck"]],
        forbidden_concepts=["low production caused by low production", "optimized dispatch"],
        expected_reliable_root_cause=False,
        expected_diagnosis_status=DiagnosisStatus.PROBABLE,
        expected_confidence=ConfidenceLevel.HIGH,
        mock_request_type=EvidenceRequestType.DOWNTIME,
        evaluation_level=EvaluationLevel.ROOT_CAUSE_DIAGNOSIS,
    ),
}
