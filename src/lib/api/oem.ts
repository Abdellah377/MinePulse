...
export const oemApi = {
  catalog: () => oemGet<OemCatalog>("/oem/catalog"),
  connectivity: (from?: string, to?: string, siteCode?: string) =>
    oemGet<{ rows: Record<string, unknown>[] }>(`/oem/connectivity${qs({ from, to, site_code: siteCode })}`),
  delays: (minDelaySec: number, from?: string, to?: string, siteCode?: string) =>
    oemGet<{ rows: Record<string, unknown>[] }>(
  ...
  pingFleet: (codes: string, from?: string, to?: string, siteCode?: string) =>
    oemGet<{ from: string; to: string; rows: Array<Record<string, unknown>> }>(
  ...
  diagnostic: (opts: Record<string, string | undefined>) =>
    oemGet<{ rows: Record<string, unknown>[] }>(`/oem/diagnostic${qs(opts)}`),
  errors: (opts: Record<string, string | undefined>) =>
    oemGet<{ rows: Record<string, unknown>[] }>(`/oem/errors${qs(opts)}`),
  maintenance: (opts: Record<string, string | undefined>) =>
    oemGet<{ rows: Record<string, unknown>[] }>(`/oem/maintenance-indicators${qs(opts)}`),
  anomalies: (opts: Record<string, string | undefined>) =>
    oemGet<{ rows: Record<string, unknown>[] }>(`/oem/anomalies${qs(opts)}`),
}
