...
it("API Alertes IA renders backend LangGraph output and qualitative confidence", () => {
  ...
  expect(html).not.toContain("Impact non quantifié")
  ...
  for (const section of ["Pourquoi MinePulse pense cela", "Action recommandée", "Liens utiles", "Panel IA", "Cause non déterminée", "Confiance causale", "Ouvrir l’équipement"]) expect(html).toContain(section)
  ...
  expect(panel).not.toContain("Cause non déterminée")
  expect(panel).not.toContain("Confiance causale")
  expect(panel).not.toContain("Action recommandée")
...
it("Actions IA investigates only on Investiguer, then deterministic runs after success; decisions still do not POST", () => {
  ...
  expect(source).toMatch(/shouldStartOptimizationRun/)
  expect(source).toMatch(/createOptimizationRun/)
  expect(source).not.toMatch(/createOptimizationWorkflow/)
  expect(source).toMatch(/startSharedWorkflowRequest/)
  expect(source).toMatch(/OPTIMIZATION_IN_PROGRESS_COPY/)
  expect(source).toMatch(/Loader2/)
  expect(source).toMatch(/OPTIMIZATION_STAGE_CONTEXT/)
  expect(source).not.toMatch(/Calcul des options de dispatch/)
  expect(source).not.toMatch(/setTimeout\(/)
  expect(source).not.toMatch(/sleep\(/)
  expect(source).toMatch(/OPTIMIZATION_UNAVAILABLE_COPY/)
  expect(source).not.toMatch(/, viewInput, result\?\.investigation_id\]/)
  ...
  expect(source).not.toMatch(/setSelectedId\(pickInboxSelection\(items, contextAlertId\)/)
  expect(source).toMatch(/\?\? \(selectedId \? null : inbox\[0\]\)/)
  expect(source).not.toMatch(/\?\? inbox\[0\]\s*$/m)
  expect(source).toMatch(/getInboxDetail/)
  expect(source).toMatch(/visibleOptimizationPlans/)
  expect(source).toMatch(/conservé/)
  expect(source).toMatch(/Aucun dossier non traité/)
